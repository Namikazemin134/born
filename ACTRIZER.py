#!/usr/bin/env python3
"""
RIZER ALL-IN-ONE ACCOUNT GENERATOR + AUTO ACTIVATOR
Version: 9.2 FINAL FIXED EDITION - FORCE FOLDER CREATION
"""
...
BASE_FOLDER = os.path.join(CURRENT_DIR, "RIZER")
...
██║░░██║██║███████╗███████╗██║░░██║
╚═╝░░╚═╝╚═╝╚══════╝╚══════╝╚═╝░░╚═╝
  {Fore.RED}⚡ ALL-IN-ONE GENERATOR + AUTO ACTIVATOR ⚡{Colors.RESET}
  {Fore.GREEN}✅ Generate & Activate Automatically!{Colors.RESET}
""")
...
def save_jwt_token(account_data, jwt_token, region, is_ghost=False):
    try:
        ...
        token_entry = {
            ...
            'region': "RIZER" if is_ghost else region,
...
def save_rare_account(account_data, rarity_type, reason, rarity_score, is_ghost=False):
    try:
        ...
        rare_entry = {
            ...
            'account_id': account_data.get("account_id", "N/A"),
            'name': account_data["name"],
            'region': "RIZER" if is_ghost else account_data.get('region', 'UNKNOWN'),
...
def save_couples_account(account1, account2, reason, is_ghost=False):
    try:
        ...
        couples_entry = {
            ...
            'reason': reason,
            'region': "RIZER" if is_ghost else account1.get('region', 'UNKNOWN'),
...
def generate_accounts_flow():
    ...
    if not is_ghost:
        print(f"\n{Fore.GREEN}{Colors.BRIGHT}🔥 AUTO ACTIVATION{Colors.RESET}")
        ...
        if AUTO_ACTIVATION_ENABLED:
            print_activation("Auto-activation ENABLED!")
        else:
    ...
    try:
        while any(t.is_alive() for t in threads):
            ...
            print(f"\n{Fore.CYAN}╔══════════════════════════════════════════════════╗")
            print(f"║ {Fore.YELLOW}⚡ RIZER STATUS✅{Fore.CYAN}                           ║")
            print(f"╠══════════════════════════════════════════════════╣")
...
def main_menu():
    while True:
        ...
        print(f"{get_random_color()}{Colors.BRIGHT}🚀 ALL-IN-ONE GENERATOR{Colors.RESET}")
        print(f"{Fore.GREEN}{Colors.BRIGHT}✅ Generate & Auto-Activate!{Colors.RESET}")
